...
def pong_ai(paddle_frect, other_paddle_frect, ball_frect, table_size):
    """
    Enhanced AI logic for the Pong game.
    ...
    if ball_velocity[0] > 0 and paddle_x < ball_frect.pos[0] < paddle_x + paddle_frect.size[0]:
        ball_last_hit_by_ai = True
    elif ball_velocity[0] < 0 and ball_frect.pos[0] < paddle_x:
        ball_last_hit_by_ai = False

    # Return to the middle of the board after hitting the ball
    if ball_last_hit_by_ai:
        if paddle_center < paddle_middle_y:
            return "down"
        elif paddle_center > paddle_middle_y:
            return "up"
        else:
    ...
    # If the ball is approaching the paddle, align the paddle to hit it
